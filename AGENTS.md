...
- **One checkout form, no cart page.** `/bulk-start` is the only checkout page AND the cart view. Designer redirects there post-add-to-cart with `?step=3&designId=...&productId=...&cartItemId=...`; the header cart icon (and any other "view cart" entry point) routes to `?step=3&fromCart=1` which loads all user `cart_items` and seeds one recipient per row. Each recipient row renders a thumbnail (design image + small product blank tile) so sellers see what they're shipping. The page is a thin client wrapper — all validation, rate-limit, and order creation happens server-side in `app/api/orders/create/route.ts`. **Do not add a second checkout form. Do not re-add `/cart`.** Removed `app/checkout/shipping/` on 2026-05-29 morning; removed `app/cart/` on 2026-05-29 afternoon.
- **Cart cleanup via the recipient `×`.** Removing the last recipient row pointing at a `cart_item_id` deletes that `cart_items` row via the RLS-gated Supabase client (no server endpoint). Best-effort; if it fails, `/api/orders/create`'s own cleanup picks up the orphan at next successful checkout.
